...
def cluster_graph(
    ...
    **_kwargs,
    ...
    try:
        # Existing code to transform DataFrame
        output_df[[level_to, to]] = pd.DataFrame(output_df[to].tolist(), index=output_df.index)
    except Exception as e:
        log.error(f"Error transforming DataFrame: {e}")
        # Optionally, add more diagnostics:
        log.debug(f"DataFrame info: {output_df.info()}")
        log.debug(f"First few rows: {output_df.head()}")
        raise
